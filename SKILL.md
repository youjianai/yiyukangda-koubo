...
1. 医疗安全、及时就医、不得保证疗效或教唆自疗
2. 用户额外指令，但仅在第 1 项允许的范围内
3. 经批准的锁定项与医学事实
4. 标题、数字、稀缺、反差、情绪等流量张力
5. 叙事完整、≥30% 二创和口播自然度
6. 300 字、格式和一般风格
7. 示例文本；示例永远不创设规则

标题默认原样保留。若标题把普通体征直接等同癌症/重病、承诺根治或免治疗、鼓励偏方替代就医、攻击同行，或保证不可控结果，只做**最小安全修改**，保留数字、悬念、反差和利益点；无法兼顾时停止导出，交编导裁决。
...
先 `Read references/parse_check_spec.md`，按其中契约生成内部解析状态：病症、方剂、单药材、穴位、`locks`、provenance 和专业核对结果。
...
- `verbatim`：标题、精确数字、方剂、病症、已批准证型、固定口诀等，按 `locked_text` 字面保全。
- `semantic_strength`：稀缺、情感、紧迫、身份等，保留或增强力度，但必须用自己的话重讲；不得把“保力度”误成照抄。
